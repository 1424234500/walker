 ...
 * 3.要传输的数据 {asdfasdf}
 */
public class StringJsonEncoder extends MessageToByteEncoder<String> {

    @Override
    protected void encode(ChannelHandlerContext tcx, String jsonstr, ByteBuf out) throws Exception {
        byte[] bs = jsonstr.getBytes(StringJsonDecoder.ENCODE);

        // 1.写入消息的开头的信息标志
        out.writeChar(StringJsonDecoder.START_WITH);
        // 2.写入长度
